...
class MultipleCandidates(base.TreeRule):
  """Multiple candidates pointing to the same person in the same contest."""
  ...
  def check(self):
    ...
    for contest in contests:
      ...
      for candidate_ids in candidate_ids_elements:
        candidate_ids_list.append(candidate_ids.text)
        contest_candidate_dict[contest_id] = candidate_ids_list
        person_id_dict = dict()
    error_log = []
    for candidate in candidates:
      candidate_object_id = candidate.get("objectId")
      if candidate_object_id in candidate_ids_list:
        person_id = candidate.find("PersonId").text
        if person_id not in person_id_dict.keys():
          person_id_dict[person_id] = candidate_object_id
        else:
          candidate_list = []
          candidate_list.append(person_id_dict[person_id])
          candidate_list.append(candidate_object_id)
          contest_id = list(contest_candidate_dict.keys())[list(
              contest_candidate_dict.values()).index(candidate_ids_list)]
          error_log.append(
              loggers.LogEntry(
                  "Multiple candidates in Contest {} reference Person {}."
                  " Candidates: {}".format(contest_id, person_id,
                                           candidate_list)))
    if error_log:
      raise loggers.ElectionError(error_log)
