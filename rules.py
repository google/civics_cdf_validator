...
class ElectionStartDates(date_rules.DateRule):
...
class ElectionEndDatesInThePast(date_rules.DateRule):
...
class ElectionEndDatesOccurAfterStartDates(date_rules.DateRule):
...
class DateStatusMatches(date_rules.DateRule):
...
class OfficeTermDates(date_rules.DateRule):
...
class RemovePersonAndOfficeHolderId60DaysAfterEndDate(base.TreeRule):
  ...
  def check(self):
    ...
    for office in offices:
      ...
      if term is not None:
        date_validator = date_rules.DateRule(None, None)
        limit_check = 0
        ...
        if end_date_person is not None:
          sixty_days_earlier = datetime.datetime.now() + datetime.timedelta(
              days=-60)
          partial_date_sixty_days = date_rules.PartialDate(
              sixty_days_earlier.year, sixty_days_earlier.month,
...
class UniqueStartDatesForOfficeRoleAndJurisdiction(date_rules.DateRule):
...
class SubsequentContestIdIsValidRelatedContest(date_rules.DateRule):
  ...
  def check(self, election_report_element):
    ...
    for contest_id, contest in contest_ids.items():
      ...
      if (contest_end_dates[subsequent_contest_id] is not None and
        ...
        end_delta = date_rules.PartialDate.is_older_than(contest_end_dates[
