...
class DateRuleTest(absltest.TestCase):
  ...
  def setUp(self):
    ...
    self.date_validator = date.DateRule(None, None)
    self.today = datetime.datetime.now().date()
    self.today_partial_date = date.PartialDate(self.today.year,
  ...
  def testResetsInstanceVarsToInitialState(self):
    ...
    validator_with_values = date.DateRule(None, None)
    ...
    fresh_validator = date.DateRule(None, None)
  ...
  def testProvidedDateIsNotInThePast(self):
    ...
    future_date = date.PartialDate(tomorrow.year, tomorrow.month, tomorrow.day)
  ...
  def testAddsToErrorLogIfDateInPast(self):
    past_date = date.PartialDate(2012, 1)
...
class PartialDateTest(absltest.TestCase):
  ...
  def testShouldCheckYear(self):
    partial_date_validator = date.PartialDate.init_partial_date("2021")
  ...
  def testShouldCheckYearMonth(self):
    partial_date_validator = date.PartialDate.init_partial_date("2021-04")
    self.assertEqual(2021, partial_date_validator.year)
    self.assertEqual(4, partial_date_validator.month)
    self.assertIsNone(partial_date_validator.day)

  def testShouldCheckDayMonthYear(self):
    partial_date_validator = date.PartialDate.init_partial_date("2021-10-19")
  ...
  def testShouldCheckIsOnlyYearMonthFalseDay(self):
    partial_date_day = date.PartialDate(2021, 11, 2)
    self.assertFalse(partial_date_day.is_month_date())

  def testReturnsNoneInvalidMonth(self):
    partial_date = date.PartialDate.init_partial_date("2022-32-24")
    self.assertIsNone(partial_date)

  def testReturnsNoneInvalidYear(self):
    partial_date = date.PartialDate.init_partial_date("20313")
    self.assertIsNone(partial_date)
